...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if (outputMessage ~= "") then
		doPlayerSendTextMessage(cid, MESSAGE_EVENT_ORANGE, string.format(__L(cid, "This purple surprise box contains:%s. The items were teleported to the Pokemon center."), outputMessage))
		doSendMagicEffect(getCreaturePosition(cid), EFFECT_FIREWORK_YELLOW)

	else
		doPlayerSendTextMessage(cid, MESSAGE_EVENT_ORANGE, "This purple surprise box is empty.")
