...
for i, skillArea in ipairs(SKILL_AREAS) do
    ...
    onTargetCreature_MassHeal = function(cid, target)
        if (isFriend(cid, target)) then
			doSendMagicEffect(getCreaturePosition(target), SKILL_EFFECT)
			doCreatureAddHealth(target, math.ceil(getMonsterLevel(cid) * 150))
        end
	end
	setCombatCallback(combats[i], CALLBACK_PARAM_TARGETCREATURE, "onTargetCreature_MassHeal")
