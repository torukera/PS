local SKILL_NAME = "Seed Bomb"
local INTERVAL = 500
local PROJECTILE = getPokemonSkillProjectile(SKILL_NAME)
local EFFECT = getPokemonSkillEffect(SKILL_NAME)
local ROUNDS = 3

function onTargetCreature_SeedBomb(cid, target)
    doSkillDamage(cid, target, SKILL_NAME, getPokemonSkillDamage(SKILL_NAME) / ROUNDS)
end

local combat = createCombatObject() setCombatCallback(combat, CALLBACK_PARAM_TARGETCREATURE, "onTargetCreature_SeedBomb")
