...
function creatureSayCallback(cid, type, msg)
	...
	if (getCatchDificultyByLevel(getPlayerLevel(cid)) < PCATCH_DIFFICULTY_EASY) then
		selfSay("Olá treinador, você precisa ter pelo menos lvl 20 para iniciar uma tarefa de captura.", cid)
		talkState[talkUser] = 0
	return true
	end
	...
	if (msgcontains(msg, 'tarefa')) then
		if (hasCatchStarted(cid) == true) then
			selfSay("Desculpe, você não pode iniciar outra tarefa de captura até que entregue a tarefa iniciada... ("..getStartedCatch(cid)..")", cid)
			talkState[talkUser] = 0
		elseif getCatchCooldown(cid) > 1 then
			...
			if (getFirstCatchPokemon(cid) and getSecondCatchPokemon(cid)) then
				selfSay("Vamos começar com uma captura fácil, você pode tentar pegar um "..getFirstCatchPokemon(cid).." ou um "..getSecondCatchPokemon(cid)..", qual você quer tentar?", cid)
				talkState[talkUser] = 1
			else
				selfSay("Desculpe, você precisa ter pelo menos lvl 20 para iniciar uma tarefa de captura.", cid)
				talkState[talkUser] = 0
			end
		end
