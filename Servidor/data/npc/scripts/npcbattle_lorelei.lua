...
local npcBattle = NpcBattle:new(getNpcName(), 9287, 9687, npcHandler)
npcBattle:setPokemons({"Dewgong", "Cloyster", "Slowbro", "Jynx", "Lapras", "Jynx"})
...
npcBattle:setOnEnd(function(cid, playerWin, npc)
	if (playerWin) then
		...
		doPlayerAchievementCheck(cid, ACHIEVEMENT_IDS.ELITE_FOUR_LORELEI)
	else
...
function onCreatureMove(cid, oldPos, newPos)
	if (isPlayer(cid) and not talking and getDistanceBetween(getNpcPos(), newPos) <= 8) then
		talking = true
		EliteFour.doMemberTalk(cid, getNpcId(), EliteFour.MEMBER_IDS.LORELEI, npcBattle)
	end
end
