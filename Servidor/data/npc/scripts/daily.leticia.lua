...
function creatureSayCallback(cid, type, msg)
	...
	if (getTaskDificultyByLevel(getPlayerLevel(cid)) > DIFFICULTY_MEDIUM) then
	...
	if (getTaskDificultyByLevel(getPlayerLevel(cid)) < DIFFICULTY_MEDIUM) then
		selfSay("Olá treinador, você precisa ser pelo menos nível 50 para fazer uma tarefa comigo.", cid)
		talkState[talkUser] = 0
	return true
	end
	...
	if (msgcontains(msg, 'iniciar')) then
		...
		elseif getTaskCooldown(cid) > 1 then
			...
			if (getFirstTaskPokemon(cid) and getSecondTaskPokemon(cid)) then
				selfSay("Nesse nível médio você pode derrotar ( "..getFirstTaskPokemon(cid).." ) ou ( "..getSecondTaskPokemon(cid).." ) qual você escolhe ?", cid)
				talkState[talkUser] = 1
			else
				selfSay("Desculpe, você precisa ter pelo menos nível 50 para fazer uma tarefa comigo.", cid)
				talkState[talkUser] = 0
			end
		end
...
npcHandler:setMessage(MESSAGE_GREET, "Olá, eu posso te dar uma tarefa diária para derrotar Pokémons! Pode ajudá-lo a treinar sua experiência de batalha. Eu posso te dar três missões no nível Médio por dia. Oque você deseja ( iniciar ) ou ( entregar )?")
