...
function creatureSayCallback(cid, type, msg)
	...
	if (getTaskDificultyByLevel(getPlayerLevel(cid)) > DIFFICULTY_EASY) then
	...
	if (getTaskDificultyByLevel(getPlayerLevel(cid)) < DIFFICULTY_EASY) then
		selfSay("Olá treinador, você precisa ser pelo menos nível 20 para fazer uma tarefa comigo.", cid)
		talkState[talkUser] = 0
	return true
	end
	...
	if (msgcontains(msg, 'iniciar')) then
		if (hasTaskStarted(cid) == true) then
			selfSay("Desculpe, você não pode iniciar outra tarefa até entregar a tarefa iniciada... ("..getStartedTask(cid)..")", cid)
			talkState[talkUser] = 0
		elseif getTaskCooldown(cid) > 1 then
			...
			if (getFirstTaskPokemon(cid) and getSecondTaskPokemon(cid)) then
				selfSay("Nesse nível fácil você pode derrotar ( "..getFirstTaskPokemon(cid).." ) ou ( "..getSecondTaskPokemon(cid).." ) qual você escolhe ?", cid)
				talkState[talkUser] = 1
			else
				selfSay("Desculpe, você precisa ter pelo menos nível 20 para fazer uma tarefa comigo.", cid)
				talkState[talkUser] = 0
			end
		end
...
npcHandler:setMessage(MESSAGE_GREET, "Olá, eu posso te dar uma tarefa diária para derrotar Pokémons! Pode ajudá-lo a treinar sua experiência de batalha. Eu posso te dar três missões no nível Fácil por dia. Oque você deseja ( iniciar ) ou ( entregar )?")
