...
function creatureSayCallback(cid, type, msg)
	...
	if (getCatchDificultyByLevel(getPlayerLevel(cid)) < PCATCH_DIFFICULTY_MEDIUM) then
		selfSay("Olá treinador, você precisa ter pelo menos lvl 50 para iniciar uma tarefa de captura.", cid)
		talkState[talkUser] = 0
	return true
	end
	...
	if (msgcontains(msg, 'tarefa')) then
		...
		elseif getCatchCooldown(cid) > 1 then
			...
			if (getFirstCatchPokemon(cid) and getSecondCatchPokemon(cid)) then
				selfSay("Vamos começar com uma captura normal, você pode tentar pegar um "..getFirstCatchPokemon(cid).." ou um "..getSecondCatchPokemon(cid)..", qual você quer tentar?", cid)
				talkState[talkUser] = 1
			else
				selfSay("Desculpe, você precisa ter pelo menos lvl 50 para iniciar uma tarefa de captura.", cid)
				talkState[talkUser] = 0
			end
		end
