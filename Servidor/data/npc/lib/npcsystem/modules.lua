...
if(Modules == nil) then
    local function error(...)
        log(LOG_TYPES.ERROR, string.concat(...))
    end
    ...
    function StdModule.say(cid, message, keywords, parameters, node)
        ...
        npcHandler:say(npcHandler:parseMessage(parameters.text or parameters.message, parseInfo, cid), cid, parameters.publicize and true)
    ...
    function FocusModule.messageMatcher(keywords, message)
        ...
        for i, word in pairs(keywords) do
            if(type(word) == 'string') then
                local findStart, findEnd = string.find(message, word)
                if (findStart and findEnd and findStart == 1 and findEnd == message:len()) then -- Exact match
    ...
    function TravelModule:parseParameters()
        ...
        if(ret ~= nil) then
            ...
            for _, word in ipairs({'destination', 'list', 'where', 'travel', 'destino', 'lista', 'viagem', 'viajar'}) do
    ...
    function TravelModule:parseDestinations(data)
        for destination in string.gmatch(data, '[^;]+') do
            local i, name, pos, cost, premium, storageId, storageValue, requiredItem = 1, nil, {x = nil, y = nil, z = nil}, nil, true
            for tmp in string.gmatch(destination, '[^,]+') do
                ...
                elseif(i == 7) then
                    storageId = tonumber(tmp)
                    if (storageId <= 0) then
                      storageId = nil
                    end
                elseif(i == 8) then
                    storageValue = tonumber(tmp)
                elseif(i == 9) then
                    requiredItem = tonumber(tmp)
                else
            ...
            if(name ~= nil and pos.x ~= nil and pos.y ~= nil and pos.z ~= nil and cost ~= nil) then
                self:addDestination(name, pos, cost, premium, storageId, storageValue, requiredItem)
            else
    ...
    function TravelModule:addDestination(name, position, price, premium, storageId, storageValue, requiredItem)
    ...
    function TravelModule.travel(cid, message, keywords, parameters, node)
        ...
        if (parameters.requiredItem and getPlayerItemCount(cid, parameters.requiredItem) > 0) then
            module.npcHandler:say('Do you want to travel to ' .. keywords[1] .. '?', cid)
        else
            module.npcHandler:say('Do you want to travel to ' .. keywords[1] .. ' for ' .. parameters.cost .. ' dollars?', cid)
        end
        return true
    end
    ...
    function TravelModule.onConfirm(cid, message, keywords, parameters, node)
        ...
        local storage = parent.storageValue or (EMPTY_STORAGE + 1)

        --[[if (parent.requiredItem and getPlayerItemCount(cid, parent.requiredItem) <= 0) then
            module.npcHandler:say('You do not have the necessary ticket to travel!', cid)
            return true
        end]]

        if (parent.storageId == nil or getPlayerStorageValue(cid, parent.storageId) >= storage) then
            if(isPlayerPremiumCallback(cid) or not parent.premium) then
                if(not isPlayerPzLocked(cid)) then
                    if((parent.requiredItem and getPlayerItemCount(cid, parent.requiredItem) > 0) or
                            (doPlayerRemoveMoney(cid, parent.cost))) then
            ...
            module.npcHandler:say('You do not have enough access to travel there!', cid)
        end
    ...
    function TravelModule.onDecline(cid, message, keywords, parameters, node)
        ...
        module.npcHandler:say(module.npcHandler:parseMessage(module.npcHandler:getMessage(MESSAGE_DECLINE), {[TAG_PLAYERNAME] = getCreatureName(cid)}, cid), cid)
        module.npcHandler:resetNpc()
        return true
    end
    ...
    function TravelModule.listDestinations(cid, message, keywords, parameters, node)
        ...
        local msg = {}
        msg[1] = __L(cid, "Your options are: ")
        for _, destination in ipairs(module.destinations) do
            ...
            msg[#msg + 1] = "{"
            msg[#msg + 1] = destination
            msg[#msg + 1] = "}"
        end

        msg[#msg + 1] = "."

        module.npcHandler:say(table.concat(msg), cid)
        module.npcHandler:resetNpc()
        return true
    end
    ...
    function ShopModule:parseSellable(data)
        for item in string.gmatch(data, '[^;]+') do
            ...
            if (itemid ~= nil and cost == nil) then
                if (getItemSellPrice(itemid) > 0) then
                    cost = getItemSellPrice(itemid)
                end
            end
    ...
    function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
        if (not self.npcHandler:isInRange(cid)) then
            return false
         end
        ...
        if (amount <= 0) then
            print('[Warning - ' .. getCreatureName(getNpcId()) .. '] NpcSystem:', 'ShopModule.onBuy - Attempt to purchase ' .. amount .. ' items' .. '. Player: ' .. getCreatureName(cid))
            return false
        end

        local backpack, totalCost = 12282, amount * shopItem.buy
        if(inBackpacks) then
        ...
        if(getPlayerMoney(cid) < totalCost) then
            ...
            doPlayerSendCancel(cid, self.npcHandler:parseMessage(msg, parseInfo, cid))
            return false
        end
        ...
        doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, self.npcHandler:parseMessage(msg, parseInfo, cid))
    ...
    function ShopModule:callbackOnSell(cid, itemid, subType, amount, ignoreCap, inBackpacks)
        ...
        if(shopItem == nil) then
            error("[ShopModule.onSell]", "Item not found on shopItems list")
            return false
        end
    ...
    function ShopModule.requestTrade(cid, message, keywords, parameters, node)
        ...
        if(table.maxn(module.npcHandler.shopItems) == 0) then
            ...
            local msg = module.npcHandler:parseMessage(module.npcHandler:getMessage(MESSAGE_NOSHOP), parseInfo, cid)
        ...
        local parseInfo = { [TAG_PLAYERNAME] = getPlayerName(cid) }
        local msg = module.npcHandler:parseMessage(module.npcHandler:getMessage(MESSAGE_SENDTRADE), parseInfo, cid)
    ...
    function ShopModule.onConfirm(cid, message, keywords, parameters, node)
        ...
        if(parentParameters.eventType == SHOPMODULE_SELL_ITEM) then
            ...
            if(ret) then
                ...
                msg = module.npcHandler:parseMessage(msg, parseInfo, cid)
                module.npcHandler:say(msg, cid)
            else
    ...
    function ShopModule.onDecline(cid, message, keywords, parameters, node)
        ...
        local msg = module.npcHandler:parseMessage(module.noText, parseInfo, cid)
        module.npcHandler:say(msg, cid)
        module.npcHandler:resetNpc()
        return true
    end
