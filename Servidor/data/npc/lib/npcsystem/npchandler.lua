...
if(NpcHandler == nil) then
    ...
    NpcHandler = {
        ...
        messages = {
            ...
            [MESSAGE_ONSELL] 	= 'Thank you for this |ITEMNAME|, |PLAYERNAME| dollars.',
            [MESSAGE_SOLD]	 	= 'Sold |ITEMCOUNT|x |ITEMNAME| for |TOTALCOST| dollars.',
            ...
            [MESSAGE_PLACEDINQUEUE] = '|PLAYERNAME|, please wait for your turn. There are |QUEUESIZE| people before you.'
        },
        convBehavior = NPCHANDLER_CONVBEHAVIOR,
        story = {},
        talkState = {}
    }

    -- Creates a new NpcHandler with an empty callbackFunction stack.
    function NpcHandler:new(keywordHandler, conversationBehavior)
        if (not conversationBehavior) then
            conversationBehavior = NPCHANDLER_CONVBEHAVIOR
        end

        local obj = {}
        obj.callbackFunctions = {}
        obj.modules = {}
        if (conversationBehavior ~= CONVERSATION_DEFAULT) then
            obj.focuses = {}
            obj.talkStart = {}
        else
        ...
        obj.messages = {}
        obj.shopItems = {}
        obj.convBehavior = conversationBehavior
    ...
    function NpcHandler:addFocus(newFocus)
        --if(NPCHANDLER_CONVBEHAVIOR ~= CONVERSATION_DEFAULT) then
        if(self.convBehavior ~= CONVERSATION_DEFAULT) then
    ...
    function NpcHandler:releaseFocus(focus)
        ...
        self:cancelStory(focus)
        self.talkState[(NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or focus)] = nil
    end
    ...
    function NpcHandler:parseMessage(msg, parseInfo, cid)
        local ret = __L(cid, msg)
        ...
        if(callback == nil or callback(cid)) then
            if(self:processModuleCallback(CALLBACK_FAREWELL)) then
                if(self.queue == nil or not self.queue:greetNext()) then
                    ...
                    msg = self:parseMessage(msg, parseInfo, cid)
    ...
    local stripAccents = string.stripAccents -- just to optmize
        ...
        if(callback == nil or callback(cid, class, msg)) then
            if(self:processModuleCallback(CALLBACK_CREATURE_SAY, cid, class, msg)) then
                ...
                if(self.keywordHandler ~= nil) then
                    ...
                    if((self:isFocused(cid) and class == TALKTYPE_PRIVATE_PN) or not self:isFocused(cid)) then
                        self:cancelStory(cid)
                    ...
                    elseif(self:isFocused(cid) and self.convBehavior == CONVERSATION_DEFAULT) then
                        self:cancelStory(cid)
    ...
    function NpcHandler:onPlayerEndTrade(cid)
        ...
        if(callback == nil or callback(cid)) then
            if(self:processModuleCallback(CALLBACK_PLAYER_ENDTRADE, cid, class, msg)) then
                if(self:isFocused(cid)) then
                    local parseInfo = { [TAG_PLAYERNAME] = getPlayerName(cid) }
                    local msg = self:parseMessage(self:getMessage(MESSAGE_ONCLOSESHOP), parseInfo, cid)
                    self:say(msg, cid)
                end
            end
        end
    ...
    function NpcHandler:onGreet(cid)
        if(self:isInRange(cid)) then
            ...
            if(self.convBehavior == CONVERSATION_PRIVATE) then
                if(not self:isFocused(cid)) then
                    self:greet(cid)
                    return true
                end
                --elseif(NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT) then
            elseif(self.convBehavior == CONVERSATION_DEFAULT) then
            ...
            if(callback == nil or callback(cid)) then
                if(self:processModuleCallback(CALLBACK_CREATURE_DISAPPEAR, cid)) then
                    if(self.queue == nil or not self.queue:greetNext()) then
                        ...
                        local parseInfo = { [TAG_PLAYERNAME] = getPlayerName(cid) or "customer" }
                        msg = self:parseMessage(msg, parseInfo, cid)
    ...
    function NpcHandler:cancelStory(cid)
        if (self.story[cid]) then
            for _, event in pairs(self.story[cid]) do
                if (os.time() < event.expire) then
                    stopEvent(event.id)
                end
                event = nil
            end
    ...
    function NpcHandler:sayStory(msg, player)
        self:cancelStory(player)
        ...
        for i, message in pairs(msg) do
            if (i == 1) then
                selfSay(message, player)
            else
                local tmpInterval = (i - 1) * 5000
                events[#events + 1] = {
                    id = addEvent(function(cid, text, player)
                        if (isNpc(cid) and isPlayer(player)) then
                            doCreatureSay(cid, text, TALKTYPE_PRIVATE_NP, false, player, getThingPos(cid))
                        end
                    end, tmpInterval, npc, message, player),
                    expire = os.time() + (tmpInterval / 1000)
                }
            end
        end

        self.story[player] = events
    end
end
