...
function onSay(cid, words, param, channel)
	...
	elseif(getPlayerGUIDByName(t[1], true) ~= nil) then -- nome ja em uso
		...
		textCancel = "close"
	elseif(getPlayerStorageValue(cid, config.item.Id) < config.item.count) then -- sem o item requerido
		local response = {value = "NameComand",text = {str = "error/nopokecoin",},}
		doSendPlayerExtendedOpcode(cid, 27, json.encode(response))
		textCancel = "close"
    ...
    local pkcoin = getPlayerStorageValue(cid, 414141) doCreatureSetStorage(cid, config.item.Id, pkcoin-config.item.count)
