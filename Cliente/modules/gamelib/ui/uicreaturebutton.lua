...
UICREATUREBUTTON_USESQUARED = false
...
function UICreatureButton:update()
    ...
    if self.isHovered or self.isTarget or self.isFollowed then
        if (UICREATUREBUTTON_USESQUARED) then
            ...
            local crosshair = nil
            if self.isTarget then
                crosshair = '/images/ui/crosshair-target'
            elseif self.isFollowed then
                crosshair = '/images/ui/crosshair-follow'
            else
                crosshair = '/images/ui/crosshair-idle'
            end

            self.creature:setCrosshairTexture(crosshair)
            self:getChildById('crosshair'):setImageSource(crosshair)
            self:getChildById('crosshair'):setVisible(true)
        end
    else
        self.creature:setCrosshairTexture('~')
        self.creature:hideStaticSquare()
        self:getChildById('creature'):setBorderWidth(0)
        self:getChildById('label'):setColor(color)
        self:getChildById('crosshair'):setVisible(false)
    end
end
