...
function g_effects.startBlink(widget, duration, interval, clickCancel)
  duration = duration and duration or 0 -- until stop is called
  interval = interval and interval or 500
  clickCancel = clickCancel and clickCancel or true

  removeEvent(widget.blinkEvent)
  removeEvent(widget.blinkStopEvent)

  widget.blinkEvent = cycleEvent(function()
    widget:setOpacity(widget:getOpacity() > 0.5 and 0.5 or 1.0)--widget:setOn(not widget:isOn())
  end, interval)
