...
function UITable:addHeader(data)
  ...
  local columns = {}
  for colId, columnInfo in ipairs(data) do
    local col = g_ui.createWidget(self.headerColumnBaseStyle)
    col.colId = colId
    col.table = self
	if columnInfo.width then col:setWidth(columnInfo.width) end
	if columnInfo.height then col:setWidth(columnInfo.height) end
	if columnInfo.text then col:setText(columnInfo.text) end
	if columnInfo.onClick then col.onClick = columnInfo.onClick col:enable() end
...
function UITable:addRow(data, height)
  ...
  for colId, column in pairs(data) do
    ...
    if column.width then col:setWidth(column.width) else col:setWidth(self.columnWidth[colId] or self.defaultColumnWidth) end
    ...
    if column.sortvalue then col.sortvalue = column.sortvalue else col.sortvalue = column.text or 0 end
	if column.align then col:setTextAlign(column.align) else col:setTextAlign(AlignLeftCenter)end
	if column.itemid then 
      local itemCol = g_ui.createWidget("BuyTableUIItem", col)
	  if column.tooltip then itemCol:setTooltip(column.tooltip) end
      itemCol:setItemId(column.itemid)
	  itemCol:setItemCount(column.count)
	  if column.poke_info and TABLE_POKEMON_INFO[column.poke_info.name] then
	    itemCol:getChildById('poke'):setImageSource("/data/images/pokeicons/"..column.poke_info.name)
	    itemCol:getChildById('level'):setText("Lv."..column.poke_info.level)
	    itemCol:getChildById('sex'):setImageSource("/data/images/game/skulls/skull_".. (column.poke_info.sex == 0 and "red" or (column.poke_info.sex == 1 and "black" or "yellow")))
	    if string.find(column.poke_info.name, "Shiny") then itemCol:getChildById('shiny'):setImageSource("/data/images/game/npcicons/icon_star") end
	    else
	    itemCol:getChildById('poke'):setImageSource("/images/ui/item")
	    itemCol:getChildById('level'):setText("")
	    itemCol:getChildById('sex'):setImageSource("/images/ui/item")
	    itemCol:getChildById('shiny'):setImageSource("/images/ui/item")
	  end
	  if column.mouseRelease then
	    itemCol.onMouseRelease = column.mouseRelease
	  end
	end
	if column.button then
	  ...
	  -- buttonColumn:setText(column.bText)
