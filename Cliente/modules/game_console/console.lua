...
local returnToWASD, skipEnterUp
local privateMessageAlertEnabled = true
...
function init()
  ...
  g_keyboard.bindKeyPress('Ctrl+Tab', function() consoleTabBar:selectNextTab() end, consolePanel)
  ...
  privateMessageAlertEnabled = g_settings.getBoolean('private-message-alert', true)
...
function terminate()
  ...
  g_settings.set('private-message-alert', privateMessageAlertEnabled)
  privateMessageAlertEnabled = nil
end
...
function addTab(name, focus)
  ...
  if tab then -- is channel already open
    if not focus and not table.find(ignoredChannels, tab.channelId) then focus = true end -- Doesnt focus a reopening channel, this happens when the player logins and the server is sending a channel that is already open, eg.: Help
  else
...
function addPrivateChannel(receiver)
  ...
  local t = addTab(receiver, false)
  if (t) then
    consoleTabBar:selectTab(t)
  end
end

function setPrivateMessageAlert(value)
    privateMessageAlertEnabled = value
end
...
--    local consoleBuffer = npcChatWindow:getChildById('consoleBuffer')
...
--    end
--
--    npcChatWindow:getChildById('npcName'):setText(creatureName)
--
--    for i, c in ipairs(g_map.getSpectators(g_game.getLocalPlayer():getPosition(), false)) do
--      if (c:getName() == creatureName) then
--        npcChatWindow:getChildById('npcCreatureBox'):setCreature(c)
--        break
--      end
--    end
  end
  ...
  if (privateMessageAlertEnabled and not g_window.hasFocus()) then
    g_sounds.getChannel(SoundChannels.Effect):play("/sounds/alert.ogg", 0,
        g_sounds.getChannel(SoundChannels.Effect):getGain())
  end
end
...
function loadCommunicationSettings()
  ...
  communicationSettings.useIgnoreList = g_settings.getBoolean('UseIgnoreList', true)
...
function doFocusNPCTab()
  local npcTab = consoleTabBar:getTab('NPCs')
  if (npcTab) then
    consoleTabBar:selectTab(npcTab)
  elseif (defaultTab) then
