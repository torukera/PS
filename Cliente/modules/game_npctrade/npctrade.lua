...
function onTradeClick()
  if getCurrentTradeType() == BUY then
    g_game.buyItem(selectedItem.ptr, quantityScroll:getValue(), true, buyWithBackpack:isChecked())
  else
...
function refreshItem(item)
  ...
  if getCurrentTradeType() == BUY then
    ...
    if (item.ptr:isStackable()) then
        ...
        finalCount = math.max(0, math.min(getMaxAmount(), math.min(priceMaxCount, capacityMaxCount)))
    else
        finalCount = 1
    end
...
function formatCurrency(amount)
  if CURRENCY_DECIMAL then
    return string.format("%.02f", amount/100.0) .. ' ' .. tr(CURRENCY)
  else
